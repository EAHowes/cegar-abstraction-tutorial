...
def boxes_disjoint_from_hull(box_mins, box_maxs, lp_prepared, tol=1e-9):
    """
    Vectorized exact-reject test for conv(vertices) vs axis-aligned boxes.

    Args:
        box_mins: (K, D) lower bounds
        box_maxs: (K, D) upper bounds
        lp_prepared: output of prepare_convex_hull_lp(vertices)

    Returns:
        disjoint: (K,) bool mask where True means guaranteed disjoint.
                  If half-space data is unavailable, returns all-False.
    """
